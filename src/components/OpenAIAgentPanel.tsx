...
export interface OpenAIAgentPanelProps {
  sessionId: string
  cwd: string
  isActive: boolean
  workspaceId?: string
...
export function OpenAIAgentPanel({ sessionId, cwd, isActive, workspaceId, onClose, showUserMsg = true, showAssistantMsg = true, showToolMsg = true, showThinkingMsg = true }: Readonly<OpenAIAgentPanelProps>) {
  ...
  // We reuse the codex flow path by keeping isCodexSession=true as "non-Claude SDK agent".
  const isCodexSession = true
  const isOpenAISession = true
  ...
  useEffect(() => {
    ...
    window.electronAPI.claude.archiveMessages(sessionId, toArchive)
      .catch((err) => {
        window.electronAPI?.debug?.log?.('[OpenAIAgentPanel] archiveMessages failed:', String(err))
      })
      ...
      >
        ...
        <div className="claude-input-footer">
          <div className="claude-input-controls">
            {isCodexSession && !isOpenAISession && (
              <>
                <select
